...
export default function FuelMap({
  ...
  // Update station markers when stations or fuelType changes
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      stations.forEach((station) => {
        ...
        const icon = L.divIcon({
          ...
          ">${price ? (price / 10).toFixed(1) + "p" : station.brand}</div>`,
          iconAnchor: [20, 12],
        });
        ...
        const marker = L.marker([station.lat, station.lng], { icon })
          ...
          .bindPopup(
            `<b>${station.brand}</b><br>${station.name}<br>${
              price
                ? `${fuelType === "petrol" ? "Petrol" : "Diesel"}: ${(price / 10).toFixed(1)}p/L`
                : "Price unavailable"
            }`
          )
